...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:choose>
      ...
      <xsl:otherwise>
        ...
        <xsl:variable name="numberSkipped">
          <xsl:choose>
            <xsl:when
              test="boolean($element[1]//step/@outcome[.='pending']) and not($element[1]//step[@outcome='failed'])">
        ...
        <xsl:variable name="totalExecuted">
          <xsl:choose>
            <xsl:when
              test="boolean($element[1][boolean(step) and not(step/ancestor::story/preceding-sibling::givenStories)])">
  ...
  <xsl:template name="example">
    ...
    <xsl:variable name="numberOfFails">
      <xsl:choose>
        <xsl:when
          test="boolean($element/following-sibling::step[@outcome='failed'][generate-id(preceding-sibling::example[1]) = generate-id($element) ])">
          <xsl:value-of select="$failures + 1" />
        </xsl:when>
        <xsl:otherwise>
          <xsl:value-of select="$failures" />
    ...
    <xsl:variable name="numberSkipped">
      <xsl:choose>
        ...
        <xsl:when
          test="boolean($element/following-sibling::step[@outcome='pending'][generate-id(preceding-sibling::example[1]) = generate-id($element) ])">
          <xsl:value-of select="$ignores + 1" />
        </xsl:when>
        <xsl:otherwise>
          <xsl:value-of select="$ignores" />
  ...
  <xsl:template name="reportBody">
    ...
    <testsuite name="{$storyTitle}" tests="{$testCount}" time="0" failures="{$numberOfFails}" errors="0"
        ...
        <xsl:choose>
          <xsl:when test="boolean(child::examples)">
            <xsl:for-each select="child::examples/example">
              ...
              <xsl:choose>
                <xsl:when
                  test="boolean(following-sibling::step[@outcome='failed'][generate-id(preceding-sibling::example[1]) = generate-id(current()) ])">
                  <testcase name="{$scenarioName} {$parameters}">
                    <xsl:apply-templates select="following::step[@outcome='failed'][1]" />
                  </testcase>
                </xsl:when>
                <xsl:when
                  test="boolean(following-sibling::step[@outcome='pending'][generate-id(preceding-sibling::example[1]) = generate-id(current()) ])">
                  <testcase name="{$scenarioName} {$parameters}">
