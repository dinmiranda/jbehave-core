...
public abstract class AbstractEmbedderTask extends Task {
    ...
    protected EmbedderControls embedderControls() {
        EmbedderControls embedderControls = new EmbedderControls().doBatch(this.batch).doSkip(this.skip)
                .doGenerateViewAfterStories(this.generateViewAfterStories).doIgnoreFailureInStories(this.ignoreFailureInStories)
                .doIgnoreFailureInView(this.ignoreFailureInView).doVerboseFailures(this.verboseFailures)
                .doVerboseFiltering(this.verboseFiltering)
                .doFailOnStoryTimeout(this.failOnStoryTimeout).useThreads(this.threads);
        if (this.storyTimeoutInSecs != 0) {
            embedderControls.useStoryTimeoutInSecs(this.storyTimeoutInSecs);
        }
        if (this.storyTimeoutInSecsByPath != null) {
            embedderControls.useStoryTimeoutInSecsByPath(this.storyTimeoutInSecsByPath);
        }
    ...
    protected List<String> storyPaths() {
        log("Searching for story paths including " + this.includes + " and excluding " + this.excludes, MSG_DEBUG);
        List<String> storyPaths = newStoryFinder().findPaths(searchDirectory(), this.includes, this.excludes);
    ...
    protected List<String> classNames() {
        log("Searching for class names including " + this.includes + " and excluding " + this.excludes, MSG_DEBUG);
        List<String> classNames = newStoryFinder().findClassNames(searchDirectory(), this.includes, this.excludes);
    ...
    protected StoryFinder newStoryFinder() {
        return classLoader().newInstance(StoryFinder.class, this.storyFinderClass);
    }
    ...
    protected Embedder newEmbedder() {
        ...
        if (this.injectableEmbedderClass != null) {
            embedder = classLoader.newInstance(InjectableEmbedder.class, this.injectableEmbedderClass).injectedEmbedder();
        } else {
            embedder = classLoader.newInstance(Embedder.class, this.embedderClass);
        }
